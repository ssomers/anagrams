...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = (w groupBy ((c: Char) => c.toLower) mapValues (o => o.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(List())
    case (c: Char, maxo: Int) :: rest => {
      val crest: List[Occurrences] = combinations(rest)
      (for (o <- 1 to maxo; cr <- crest) yield ((c, o) :: cr)).toList ++ crest
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (x, y) match {
    ...
    case ((xc: Char, xo: Int) :: xr, (yc: Char, yo: Int) :: yr) =>
      if (xc == yc) if (xo == yo) subtract(xr, yr) else (xc, xo - yo) :: subtract(xr, yr) else (xc, xo) :: subtract(xr, y)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    val dbo = dictionaryByOccurrences

    def subsentencesAfterPrefix(occurrences: Occurrences, suboccurrences: Occurrences): List[Sentence] = {
      val remainingOccurrences = subtract(occurrences, suboccurrences)
      if (remainingOccurrences == Nil) List(List())
      else {
        val ss = subsentences(remainingOccurrences)
        ss
      }
    }
